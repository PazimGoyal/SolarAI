...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const locate_marker = document.getElementById(
    "locate_marker"
  ...
  lat_long_form.addEventListener("submit", (event: SubmitEvent) => {
    ...
    try {
      const response = fetch("http://localhost:8000", {
        ...
        .then((res) => res.json())
        .then((res) => {
          const output: HTMLDivElement = document.getElementById(
            "output"
          ) as HTMLDivElement;
          output.innerHTML = "";
          ...
          if (list) {
            let inner =
              "<h2>Solar insolation intensity estimates</h2><table><tr><th>DATE TIME</th><th>GHI</th><th>DHI</th><th>DNI</th><tr>";
  ...
  function plotlines(latitude: number, longitude: number, azimuth: number) {
    const length = 0.001;
